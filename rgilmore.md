...
It's relatively easy to update of the participant metadata file by exporting the data as a .csv after they have been entered into Databrary. From there the unnecessary columns for a particular analysis (e.g. Race, Ethnicity, Task name) can be dropped, leaving only the participant ID, test date, day age, and gender columns.
